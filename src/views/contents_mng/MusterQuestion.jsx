...
const MusterQuestion = () => {
  ...
  const {
    data: musterQuestion,
    isLoading: isMusterQuestion,
    refetch: refetchMusterQuestion,
  } = useQuery(
    'getMusterQuestion',
    () =>
      request.get(`/admin/content-management/muster-question`, {
        params: {
      ...
      <div className="intro-y box mt-5 relative">
        <div className="p-5">
          ...
          <table className="table table-hover mt-5">
            ...
            <tbody>
            {musterQuestion?.content?.length > 0 ? (
              musterQuestion?.content?.map((item, index) => (
                <tr className="text-center" key={index}>
                  <td>{musterQuestion?.content?.length - index}</td>
                  <td>{item.year}</td>
