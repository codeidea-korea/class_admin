...
function PwFindstep1() {
    ...
    window.addEventListener('message', (event) => {
      if (event.origin === window.location.origin) {
        ...
        if (result.userPhone !== null && result.userPhone !== '') {
          setCheckAuth(true)
          setFindPwdParams({ ...findPwdParams, phone: result.userPhone })
        }
      }
    })
  }
  ...
  const linkResetPwd = () => {
    if (checkAuth) {
      navigate('/pw_find_step2', {
        state: {
          userId: findPwdParams.userId,
          name: findPwdParams.name,
          phone: findPwdParams.phone,
        },
      })
    } else {
                  ...
                  <div className='mt-3'>
                    ...
                    <div className='flex gap-2 items-center mt-1'>
                      <input type='text' className='intro-x login__input form-control py-3 px-4 block mt-1'
                             ...
                             disabled={true}
                      />
                      <button className='btn btn-danger w-20 py-3 shrink-0' onClick={jsSubmit}>인증요청</button>
